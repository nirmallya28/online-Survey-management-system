...
if(@$_GET['fdid'] && $_SESSION['key']=='77975866307785068889') {
$id=@$_GET['fdid'];
...
//delete user
if(isset($_SESSION['key'])){
if(@$_GET['demail'] && $_SESSION['key']=='77975866307785068889') {
$demail=@$_GET['demail'];
...
//remove survey
if(isset($_SESSION['key'])){
if(@$_GET['q']== 'rmsurvey' && $_SESSION['key']=='77975866307785068889') {
$eid=@$_GET['eid'];
...
$r4 = mysqli_query($con,"DELETE FROM survey WHERE eid='$eid' ") or die('Error');
...
//add survey
if(isset($_SESSION['key'])){
if(@$_GET['q']== 'addsurvey' && $_SESSION['key']=='77975866307785068889') {
$name = $_POST['name'];
...
$desc = $_POST['desc'];
$id=uniqid();
$q3=mysqli_query($con,"INSERT INTO survey VALUES  ('$id','$name' , '$sahi' , '$wrong','$total','$time' ,'$desc','$tag', NOW())");
...
//add question
if(isset($_SESSION['key'])){
if(@$_GET['q']== 'addqns' && $_SESSION['key']=='77975866307785068889') {
$n=@$_GET['n'];
$eid=@$_GET['eid'];
$ch=@$_GET['ch'];
...
if($ans == $ansid)
{
$q=mysqli_query($con,"SELECT * FROM survey WHERE eid='$eid' " );
while($row=mysqli_fetch_array($q) )
{
$sahi=$row['sahi'];
}
...
{
$q=mysqli_query($con,"SELECT * FROM survey WHERE eid='$eid' " )or die('Error129');
...
{
$sn++;
header("location:account.php?q=survey&step=2&eid=$eid&n=$sn&t=$total")or die('Error152');
}
else if( $_SESSION['key']!='77975866307785068889')
{
...
//restart survey
if(@$_GET['q']== 'surveyre' && @$_GET['step']== 25 ) {
$eid=@$_GET['eid'];
$n=@$_GET['n'];
$t=@$_GET['t'];
$q=mysqli_query($con,"SELECT score FROM history WHERE eid='$eid' AND email='$email'" )or die('Error156');
...
$sun=$sun-$s;
$q=mysqli_query($con,"UPDATE `rank` SET `score`=$sun ,time=NOW() WHERE email= '$email'")or die('Error174');
header("location:account.php?q=survey&step=2&eid=$eid&n=1&t=$t");
}
